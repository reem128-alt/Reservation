...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      router.push(
        `/${locale}/auth/verify-otp?email=${encodeURIComponent(response.email)}&purpose=LOGIN`
      )
    } catch (err: unknown) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <p className="text-center text-muted-foreground">
         {" Don't have an account?"}{" "}
          <Link href={`/${locale}/auth/register`} className="text-accent hover:text-accent/80 font-semibold transition-colors">
            Create account
          </Link>
        </p>
      </form>
