  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) {
      toastApiError(
        "Verification failed",
        null,
        "Missing email. Please login again."
      );
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <Button
          ...
          className="w-full h-12 text-base font-medium"
          onClick={() => {
            const purposeFromQuery = new URLSearchParams(
            ...
            const purpose =
              purposeFromQuery === "LOGIN" || purposeFromQuery === "REGISTER"
                ? purposeFromQuery
                : purposeFromStorage === "LOGIN" ||
                  purposeFromStorage === "REGISTER"
                ? purposeFromStorage
                : "REGISTER";
            router.push(
              `/${locale}/auth/${purpose === "LOGIN" ? "login" : "register"}`
            );
          }}
        >
